...
package com.binishmatheww.bubble.core.ble
...
object ConnectionManager {
    ...
    fun teardownConnection(device: BluetoothDevice) {
        if (device.isConnected()) {
            ...
            log("Not connected to ${device.address}, cannot teardown connection!")
        }
    }
    ...
    fun readCharacteristic(device: BluetoothDevice, characteristic: BluetoothGattCharacteristic) {
        if (device.isConnected() && characteristic.isReadable()) {
            ...
            log("Attempting to read ${characteristic.uuid} that isn't readable!")
        } else if (!device.isConnected()) {
            log("Not connected to ${device.address}, cannot perform characteristic read")
        }
    }
    ...
    fun writeCharacteristic(
        ...
        val writeType = when {
            ...
            characteristic.isWritableWithoutResponse() -> {
                ...
                log("Characteristic ${characteristic.uuid} cannot be written to")
                return
            }
        }
        if (device.isConnected()) {
            ...
            log("Not connected to ${device.address}, cannot perform characteristic write")
        }
    }
    ...
    fun readDescriptor(device: BluetoothDevice, descriptor: BluetoothGattDescriptor) {
        if (device.isConnected() && descriptor.isReadable()) {
            ...
            log("Attempting to read ${descriptor.uuid} that isn't readable!")
        } else if (!device.isConnected()) {
            log("Not connected to ${device.address}, cannot perform descriptor read")
        }
    }
    ...
    fun writeDescriptor(
        ...
        if (device.isConnected() && (descriptor.isWritable() || descriptor.isCccd())) {
            ...
            log("Not connected to ${device.address}, cannot perform descriptor write")
        } else if (!descriptor.isWritable() && !descriptor.isCccd()) {
            log("Descriptor ${descriptor.uuid} cannot be written to")
        }
    }
    ...
    fun enableNotifications(device: BluetoothDevice, characteristic: BluetoothGattCharacteristic) {
        if (device.isConnected() &&
            ...
            log("Not connected to ${device.address}, cannot enable notifications")
        } else if (!characteristic.isIndicatable() && !characteristic.isNotifiable()) {
            log("Characteristic ${characteristic.uuid} doesn't support notifications/indications")
        }
    }
    ...
    fun disableNotifications(device: BluetoothDevice, characteristic: BluetoothGattCharacteristic) {
        if (device.isConnected() &&
            ...
            log("Not connected to ${device.address}, cannot disable notifications")
        } else if (!characteristic.isIndicatable() && !characteristic.isNotifiable()) {
            log("Characteristic ${characteristic.uuid} doesn't support notifications/indications")
        }
    }
    ...
    fun requestMtu(device: BluetoothDevice, mtu: Int) {
        if (device.isConnected()) {
            ...
            log("Not connected to ${device.address}, cannot request MTU update!")
        }
    }
    ...
    private fun doNextOperation() {
        if (pendingOperation != null) {
            log("doNextOperation() called when an operation is pending! Aborting.")
            return
        }

        val operation = operationQueue.poll() ?: run {
            log("Operation queue empty, returning")
            return
        }
        ...
        val gatt = deviceGattMap[operation.device]
            ?: this@ConnectionManager.run {
                log("Not connected to ${operation.device.address}! Aborting $operation operation.")
        ...
        when (operation) {
            ...
            is CharacteristicWrite -> with(operation) {
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    log("Cannot find $characteristicUuid to write to")
                    signalEndOfOperation()
                }
            }
            is CharacteristicRead -> with(operation) {
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    log("Cannot find $characteristicUuid to read from")
                    signalEndOfOperation()
                }
            }
            is DescriptorWrite -> with(operation) {
                gatt.findDescriptor(descriptorUuid)?.let { descriptor ->
                    ...
                    log("Cannot find $descriptorUuid to write to")
                    signalEndOfOperation()
                }
            }
            is DescriptorRead -> with(operation) {
                gatt.findDescriptor(descriptorUuid)?.let { descriptor ->
                    ...
                    log("Cannot find $descriptorUuid to read from")
                    signalEndOfOperation()
                }
            }
                ...
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    characteristic.getDescriptor(cccdUuid)?.let { cccDescriptor ->
                        if (!gatt.setCharacteristicNotification(characteristic, true)) {
                            log("setCharacteristicNotification failed for ${characteristic.uuid}")
                            signalEndOfOperation()
                            return
                        }
                        ...
                        log("${characteristic.uuid} doesn't contain the CCC descriptor!")
                        signalEndOfOperation()
                    }
                } ?: this@ConnectionManager.run {
                    log("Cannot find $characteristicUuid! Failed to enable notifications.")
                    signalEndOfOperation()
                }
            }
            is DisableNotifications -> with(operation) {
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    log("Cannot find $characteristicUuid! Failed to disable notifications.")
                    signalEndOfOperation()
                }
            }
    ...
    private val callback = object : BluetoothGattCallback() {
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    log("onConnectionStateChange: connected to $deviceAddress")
                    deviceGattMap[gatt.device] = gatt
                    Handler(Looper.getMainLooper()).post {
                        gatt.discoverServices()
                    }
                } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                    log("onConnectionStateChange: disconnected from $deviceAddress")
                    teardownConnection(gatt.device)
                }
            } else {
                log("onConnectionStateChange: status $status encountered for $deviceAddress!")
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            with(gatt) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    log("Discovered ${services.size} services for ${device.address}.")
                    printGattTable()
                    requestMtu(device, GATT_MAX_MTU_SIZE)
                    listeners.forEach { it.get()?.onConnectionSetupComplete?.invoke(this) }
                } else {
                    log("Service discovery failed due to status $status")
                    teardownConnection(gatt.device)
                }
            }
        ...
        override fun onMtuChanged(gatt: BluetoothGatt, mtu: Int, status: Int) {
            log("ATT MTU changed to $mtu, success: ${status == BluetoothGatt.GATT_SUCCESS}")
            ...
            with(characteristic) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        log("Read characteristic $uuid | value: ${value.toHexString()}")
        ...
        override fun onCharacteristicWrite(
            ...
            with(characteristic) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        log("Wrote to characteristic $uuid | value: ${value.toHexString()}")
                        listeners.forEach { it.get()?.onCharacteristicWrite?.invoke(gatt.device, this) }
                    }
                    BluetoothGatt.GATT_WRITE_NOT_PERMITTED -> {
                        log("Write not permitted for $uuid!")
                    }
                    else -> {
                        log("Characteristic write failed for $uuid, error: $status")
                    }
                }
            }
        ...
        override fun onCharacteristicChanged(
            ...
            with(characteristic) {
                log("Characteristic $uuid changed | value: ${value.toHexString()}")
        ...
        override fun onDescriptorRead(
            ...
            with(descriptor) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        log("Read descriptor $uuid | value: ${value.toHexString()}")
                    ...
                    BluetoothGatt.GATT_READ_NOT_PERMITTED -> {
                        ...
                        log("Descriptor read failed for $uuid, error: $status")
                    }
                }
            }
        ...
        override fun onDescriptorWrite(
            ...
            with(descriptor) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        log("Wrote to descriptor $uuid | value: ${value.toHexString()}")
                    ...
                    BluetoothGatt.GATT_WRITE_NOT_PERMITTED -> {
                        ...
                        log("Descriptor write failed for $uuid, error: $status")
                    }
                }
            }
        ...
        private fun onCccdWrite(
            ...
            when {
                notificationsEnabled -> {
                    log("Notifications or indications ENABLED on $charUuid")
                    listeners.forEach {
                ...
                notificationsDisabled -> {
                    log("Notifications or indications DISABLED on $charUuid")
                    listeners.forEach {
                    ...
                    log("Unexpected value ${value.toHexString()} on CCCD of $charUuid")
                }
            }
        }
    }
        ...
        override fun onReceive(context: Context, intent: Intent) {
            with(intent) {
                if (action == BluetoothDevice.ACTION_BOND_STATE_CHANGED) {
                    ...
                    log("${device?.address} bond state changed | $bondTransition")
                }
            }
        }
