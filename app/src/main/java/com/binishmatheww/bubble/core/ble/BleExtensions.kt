...
fun BluetoothGatt.printGattTable() {
    if (services.isEmpty()) {
        log("No service and characteristic available, call discoverServices() first?")
        return
    }
    services.forEach { service ->
        val characteristicsTable = service.characteristics.joinToString(
            ...
            var description = "${GattUtilities.GattCharacteristics.lookup(char.uuid)}: ${char.printProperties()}"
 ...
 */
fun BluetoothGattDescriptor.isCccd() = uuid.toString().equals(CCC_DESCRIPTOR_UUID, ignoreCase = true)

// ByteArray
fun ByteArray.toHexString(): String = joinToString(separator = " ", prefix = "") { String.format("%02x", it) }
